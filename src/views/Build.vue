<template>
  <div>
    ...
    <img v-bind:src="'https://ddragon.leagueoflegends.com/cdn/9.7.1/img/champion/' + this.$props.champion + '.png'"
      alt="">
    <br>
...
<script>
  export default {
    ...
    methods: {
      genBuild() {
        for (let i = 0; i < 6; i++) {
          this.$data.build.push(
            this.$data.items[Math.floor(Math.random() * this.$data.items.length)]);
        }
